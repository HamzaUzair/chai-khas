...
export default function SeatingPage() {
  ...
  const persist = useCallback((updated: Hall[]) => {
  ...
  const handleSubmit = (data: HallFormData) => {
    ...
    if (editingHall) {
      ...
      persist(updated);
      pushToast(`Seating area "${data.name.trim()}" updated successfully.`);
    } else {
      const newHall: Hall = {
      ...
      pushToast(`Seating area "${data.name.trim()}" created successfully.`);
    }
    closeModal();
  };

  const handleDelete = (hall: Hall) => {
    if (!window.confirm(`Delete seating area "${hall.name}"? This cannot be undone.`)) return;
    persist(halls.filter((h) => h.id !== hall.id));
    pushToast(`Seating area "${hall.name}" deleted.`, "error");
  };
    ...
    <DashboardLayout title="Seating">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Seating</h2>
            <p className="text-sm text-gray-500 mt-1">
              Manage seating areas and capacity by branch
            </p>
          </div>
      ...
      {noBranches && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-5 py-3.5 mb-6 text-sm">
          ...
          <p>
            <span className="font-semibold">No branches available.</span> Create a
            branch first before adding seating areas.
          </p>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          <div className="px-6 py-16 flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-sm text-gray-500 max-w-xs">
              {halls.length === 0
                ? "No seating areas yet. Add your first one."
                : "No seating areas match your filters."}
            </p>
