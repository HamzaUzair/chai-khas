"use client";

import React from "react";
import { Search, Building2, Printer, Filter } from "lucide-react";
import type { Branch } from "@/types/branch";
import { PRINTER_TYPES } from "@/lib/constants/printerConfig";

export type PrinterStatusFilter = "active" | "inactive" | "offline" | "all";

interface PrinterToolbarProps {
  branches: Branch[];
  branchesLoading: boolean;
  filterBranchId: number | "all";
  onBranchChange: (v: number | "all") => void;
  filterType: string;
  onTypeChange: (v: string) => void;
  statusFilter: PrinterStatusFilter;
  onStatusChange: (v: PrinterStatusFilter) => void;
  search: string;
  onSearchChange: (v: string) => void;
}

const pillBase =
  "px-3.5 py-1.5 rounded-full text-xs font-semibold cursor-pointer transition-all border";

const PrinterToolbar: React.FC<PrinterToolbarProps> = ({
  branches,
  branchesLoading,
  filterBranchId,
  onBranchChange,
  filterType,
  onTypeChange,
  statusFilter,
  onStatusChange,
  search,
  onSearchChange,
}) => (
  <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6 space-y-4">
    {/* Row 1: Branch + Type + Search */}
    <div className="flex flex-col lg:flex-row lg:items-end gap-4">
      <div className="flex-1 min-w-[180px] max-w-xs">
        <label className="flex items-center gap-1.5 text-xs font-medium text-gray-500 mb-1.5">
          <Building2 size={13} />
          Branch
        </label>
        <select
          className="w-full border border-gray-200 rounded-lg px-3.5 py-2 text-sm text-gray-700 bg-white appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#ff5a1f]/30 focus:border-[#ff5a1f] transition-all"
          value={filterBranchId}
          onChange={(e) => {
            const v = e.target.value;
            onBranchChange(v === "all" ? "all" : Number(v));
          }}
          disabled={branchesLoading}
        >
          <option value="all">All Branches</option>
          {branches.map((b) => (
            <option key={b.branch_id} value={b.branch_id}>
              {b.branch_name}
            </option>
          ))}
        </select>
      </div>

      <div className="flex-1 min-w-[180px] max-w-xs">
        <label className="flex items-center gap-1.5 text-xs font-medium text-gray-500 mb-1.5">
          <Printer size={13} />
          Printer Type
        </label>
        <select
          className="w-full border border-gray-200 rounded-lg px-3.5 py-2 text-sm text-gray-700 bg-white appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#ff5a1f]/30 focus:border-[#ff5a1f] transition-all"
          value={filterType}
          onChange={(e) => onTypeChange(e.target.value)}
        >
          <option value="all">All Types</option>
          {PRINTER_TYPES.map((t) => (
            <option key={t.value} value={t.value}>
              {t.label}
            </option>
          ))}
        </select>
      </div>

      <div className="flex-1 min-w-[220px] max-w-sm">
        <label className="flex items-center gap-1.5 text-xs font-medium text-gray-500 mb-1.5">
          <Search size={13} />
          Search
        </label>
        <div className="relative">
          <Search
            size={16}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"
          />
          <input
            type="text"
            className="w-full border border-gray-200 rounded-lg pl-9 pr-3.5 py-2 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff5a1f]/30 focus:border-[#ff5a1f] transition-all"
            placeholder="Search by name, area, IP…"
            value={search}
            onChange={(e) => onSearchChange(e.target.value)}
          />
        </div>
      </div>
    </div>

    {/* Row 2: Status pills */}
    <div className="flex flex-wrap items-center gap-2">
      <Filter size={14} className="text-gray-400 shrink-0" />
      {(["active", "inactive", "offline", "all"] as PrinterStatusFilter[]).map((s) => {
        const isSelected = statusFilter === s;
        let colors: string;
        if (isSelected) {
          colors =
            s === "active"
              ? "bg-green-50 text-green-700 border-green-200"
              : s === "inactive"
              ? "bg-gray-100 text-gray-600 border-gray-200"
              : s === "offline"
              ? "bg-amber-50 text-amber-700 border-amber-200"
              : "bg-[#ff5a1f]/10 text-[#ff5a1f] border-[#ff5a1f]/20";
        } else {
          colors = "bg-white text-gray-500 border-gray-200 hover:bg-gray-50";
        }
        return (
          <button
            key={s}
            onClick={() => onStatusChange(s)}
            className={`${pillBase} ${colors}`}
          >
            {s === "active" && "● "}
            {s === "inactive" && "○ "}
            {s === "offline" && "◐ "}
            {s.charAt(0).toUpperCase() + s.slice(1)}
          </button>
        );
      })}
    </div>
  </div>
);

export default PrinterToolbar;
